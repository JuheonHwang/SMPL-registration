...
def batch_mm(matrix, matrix_batch: torch.Tensor):
    """
    ...
    vectors = matrix_batch.transpose(0, 1).flatten(1)

    # A matrix-matrix product is a batched matrix-vector product of the columns.
    # And then reverse the reshaping. (m, n) x (n, b*k) = (m, b*k) -> (m, b, k) -> (b, m, k)
    return matrix.mm(vectors).view(matrix.shape[0], batch_size, -1).transpose(1, 0).contiguous()
