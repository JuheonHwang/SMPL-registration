...
from lib.smpl.smplpytorch.smplpytorch.native.webuser.serialization import ready_arguments, backwards_compatibility_replacements
...
class SMPL_Layer(Module):
    ...
    def __init__(self,
        ...
        self.hires = hires
        if hires:
            smpl_data = self.get_hres_smpl_model_data(self.model_path)
        else:
            smpl_data = ready_arguments(self.model_path)
            smpl_data['betas'] = smpl_data['betas'].r
            smpl_data['shapedirs'] = smpl_data['shapedirs'].r
            smpl_data['posedirs'] = smpl_data['posedirs'].r
            smpl_data['v_template'] = smpl_data['v_template'].r
            smpl_data['weights'] = smpl_data['weights'].r
        ...
        self.register_buffer('th_shapedirs',
                             torch.Tensor(smpl_data['shapedirs'][:, :, :num_betas]))
    ...
    def forward(self,
        ...
        th_trans = th_trans + root_j.squeeze(2) - torch.einsum('bij,bj->bi', root_rot, root_j.squeeze(2)) #added
        th_trans[:, 0] = -1 * th_trans[:, 0]
        th_trans[:, 1] = -1 * th_trans[:, 1]
        th_jtr = th_jtr + th_trans.unsqueeze(1)
        th_verts = th_verts + th_trans.unsqueeze(1)
    ...
    def get_hres_smpl_model_data(self, smpl_file):

        dd = ready_arguments(smpl_file)
