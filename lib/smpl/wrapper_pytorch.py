...
from lib.torch_functions import batch_sparse_dense_matmul, sparse_mx_to_torch_sparse_tensor, batch_mm
from .const import *
...
class SMPLPyTorchWrapperBatch(nn.Module):
    def __init__(self, model_root, batch_sz,
                 ...
                 gender='male', num_betas=300, hands=False, hires=False,
                 device='cuda:0'):
        ...
        self.smpl = SMPL_Layer(center_idx=0, gender=gender, num_betas=num_betas,
                               model_root=str(model_root), hands=hands, hires=hires)
        ...
        if offsets is None or offsets.shape[1] == 1:
            self.offsets = nn.Parameter(torch.zeros(batch_sz, 27554, 3) if hires else torch.zeros(batch_sz, 6890, 3))
        else:
            assert offsets.ndim == 3
            if hires and offsets.shape[1] == 6890:
                mapping = sparse_mx_to_torch_sparse_tensor(self.smpl.smpl_data['mapping']).to(offsets.device)
                offsets = batch_mm(mapping, offsets)
