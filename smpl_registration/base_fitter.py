...
class BaseFitter(object):
    ...
    def init_smpl(self, batch_sz, gender, pose=None, betas=None, trans=None, offsets=None, flip=True, hires=False):
        """
        ...
        if pose is None:
            ...
            if flip:
                # pose_init[:, 2] = np.pi
                # pose_init[:, 0] = 1.8986511
                # pose_init[:, 1] = -2.583704
                # pose_init[:, 2] = -0.41352633
                ...
                pose_init[:, 50] = -8 * np.pi / 24
                pose_init[:, 53] = 8 * np.pi / 24
                # pose_init[:, 2] = 0.5 * np.pi
        else:
        ...
        offs_init = torch.zeros((batch_sz, 1, 3)) if offsets is None else offsets
        ...
        smpl = SMPLPyTorchWrapperBatch(self.model_root, batch_sz, betas, pose, trans, offs_init,
                                       num_betas=num_betas, device=self.device,
                                       gender=gender, hands=self.hands, hires=hires).to(self.device)
        return smpl
    ...
    def save_hres_outputs(self, save_path, scan_paths, hres_smpl_meshes, hres2lres_smpl_meshes, save_name_hres='smpl_hres', save_name_hres2lres='smpl_hres2lres'):
        mesh_paths_hres, _ = self.get_mesh_paths(save_name_hres, save_path, scan_paths)
        self.save_meshes(hres_smpl_meshes, mesh_paths_hres)
        mesh_paths_hres2lres, _ = self.get_mesh_paths(save_name_hres2lres, save_path, scan_paths)
        self.save_meshes(hres2lres_smpl_meshes, mesh_paths_hres2lres)
    ...
    def save_smpl_params(self, names, save_path, smpl, save_name):
        for p, b, t, o, n in zip(smpl.pose.cpu().detach().numpy(), smpl.betas.cpu().detach().numpy(),
                                 smpl.trans.cpu().detach().numpy(), smpl.offsets.detach().cpu().numpy(), names):
            smpl_dict = {'pose': p, 'betas': b, 'trans': t, 'offs': o}
            sfx = splitext(n)[1]
    ...
    def load_j3d(self, pose_files):
        ...
        for file in pose_files:
            ...
            J3d = np.array(data[0]['keypoints3d']).reshape((-1, 4))
            # J3d = np.array(data[0]['keypoints3d']) / 1000
            # J3d = J3d.reshape((-1, 4))
            joints.append(J3d)
