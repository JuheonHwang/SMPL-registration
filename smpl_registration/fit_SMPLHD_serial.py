...
class SMPLDFitter(SMPLHFitter):
    ...
    def fit(self, scans, pose_files, smpl_pkl, gender='male', save_path=None):
        ...
        pose, betas, trans = super(SMPLDFitter, self).fit([scans[0]], [pose_files[0]], [smpl_pkl[0]], gender, save_path)
        ...
        smpl = self.init_smpl(1, gender, pose, betas, trans)
        ...
        lres_mapping = torch.tensor(lres_mapping).float().to(self.device)
        for i in range(batch_sz):
            ...
            if i == 0:
                ...
                verts, _, _, _ = smpl()
                hres_optimized_meshes, hres_offsets = self.optimize_high_res(th_scan_meshes, verts, 10, 20)
                ...
                hres2lres_verts_list = [hres2lres_verts[j].clone().detach() for j in range(1)]
                hres2lres_faces_list = [smpl.faces for j in range(1)]
                hres2lres_optimized_meshes = Meshes(hres2lres_verts_list, hres2lres_faces_list)
            else:
                smpl_param = [pose, betas, trans]
                pose, betas, trans = super(SMPLDFitter, self).fit_pose([scans[i]], [pose_files[i]], smpl_param, gender, save_path)
                betas, pose, trans = torch.tensor(betas), torch.tensor(pose), torch.tensor(trans)
                smpl.trans = torch.nn.Parameter(trans.to(self.device))
                smpl.pose = torch.nn.Parameter(pose.to(self.device))
                smpl.betas = torch.nn.Parameter(betas.to(self.device))
                verts, _, _, _ = smpl()
                hres_optimized_meshes, hres_offsets = self.optimize_high_res2(th_scan_meshes, verts, hres_offsets.clone().detach(), 10, 20)
                hres_optimized_verts = torch.stack([*hres_optimized_meshes.verts_list()], 0)
                hres2lres_verts = torch.matmul(lres_mapping, hres_optimized_verts)
                hres2lres_verts_list = [hres2lres_verts[j].clone().detach() for j in range(1)]
                hres2lres_faces_list = [smpl.faces for j in range(1)]
                hres2lres_optimized_meshes = Meshes(hres2lres_verts_list, hres2lres_faces_list)
            if save_path is not None:
                self.save_hres_outputs(save_path, [scans[i]], hres_optimized_meshes, hres2lres_optimized_meshes,
    ...
    def forward_step_offset_hres2(self, th_scan_meshes, init_hres_smpl, hres_offsets, init_smpl_lap, target_lengths):
        """
            Performs a forward step, given smpl and scan meshes.
            Then computes the losses.
        """
        ...
        # reference: https://github.com/NVIDIAGameWorks/kaolin/blob/v0.1/kaolin/metrics/mesh.py#L155
        # loss['lap'] = 1.5 * torch.mean(torch.sum((lap_new - init_smpl_lap)**2, 1))
        # loss['lap'] = 6 * torch.mean(torch.sum((lap_new - init_smpl_lap) ** 2, 1))
        loss['lap'] = 1 * torch.mean(torch.sum(lap_new ** 2, 1))
        # loss['edge'] = 0.1 * mesh_edge_loss(new_hres_smpl)
        loss['edge'] = 3 * mesh_edge_loss_initial(new_hres_smpl, target_lengths)
        # loss['offsets'] = torch.mean(torch.mean(0.001 * hres_offsets ** 2, axis=1))
    ...
    def optimize_high_res(self, th_scan_meshes, verts, iterations, steps_per_iter):
        # Optimizer
        # optimizer = torch.optim.Adam([smpl.offsets, smpl.pose, smpl.betas], 0.005, betas=(0.9, 0.999))
        ...
        hres_face = torch.tensor(hres_face.astype(np.int32)).long().to(th_scan_meshes.device)
        hres_mapping = torch.tensor(hres_mapping).float().to(th_scan_meshes.device)
        ...
        return init_hres_smpl.offset_verts(hres_offsets), hres_offsets

    def optimize_high_res2(self, th_scan_meshes, verts, hres_offsets_input, iterations, steps_per_iter):
        # Optimizer
        # optimizer = torch.optim.Adam([smpl.offsets, smpl.pose, smpl.betas], 0.005, betas=(0.9, 0.999))
        high_res = pkl.load(open("../assets/SMPL_high_res_mapping.pkl", 'rb'), encoding='latin-1')
        hres_face, hres_mapping = high_res['face'], high_res['mapping']
        ...
        hres_faces_list = [hres_face for i in range(bz)]
        init_hres_smpl = Meshes(hres_verts_list, hres_faces_list)
        init_hres_smpl = init_hres_smpl.offset_verts(hres_offsets_input)
        init_hres_lap = mesh_laplacian_smoothing(init_hres_smpl)
        # init_hres_smpl_edges_packed = init_hres_smpl.edges_packed()
        # init_hres_smpl_verts_packed = init_hres_smpl.verts_packed()
        # init_hres_smpl_verts_edges = init_hres_smpl_verts_packed[init_hres_smpl_edges_packed]
        ...
        for it in range(iterations):
            ...
            for i in loop:
                optimizer.zero_grad()
                # Get losses for a forward pass
                loss_dict = self.forward_step_offset_hres2(th_scan_meshes, init_hres_smpl, hres_offsets, init_hres_lap, target_lengths)
                # Get total loss for backward pass
                tot_loss = self.backward_step(loss_dict, weight_dict, it)
                tot_loss.backward()
                optimizer.step()
...
if __name__ == "__main__":
    ...
    args.scan_path = glob.glob('../data/hjh/*.ply')
    args.pose_file = glob.glob('../data/hjh/*.json')
    args.smpl_pkl = glob.glob("../data/hjh/*.pkl")
    ...
    args.save_path = '../data/hjh_result_serial2'
    args.gender = 'neutral'
    config = load_config(args.config_path)
    args.model_root = Path(config["SMPL_MODELS_PATH"])
