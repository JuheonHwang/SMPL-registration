...
class SMPLHFitter(BaseFitter):
    def fit(self, scans, pose_files, smpl_pkl, gender='male', save_path=None):
        ...
        # iterations, pose_iterations, steps_per_iter, pose_steps_per_iter = 0, 0, 0, 0
    ...
    def fit_pose(self, scans, pose_files, smpl_params, gender='male', save_path=None):
        # Batch size
        batch_sz = len(scans)

        # init smpl
        pose, betas, trans = smpl_params[0], smpl_params[1], smpl_params[2]
        ...
        if pose_files is not None:
            ...
            # Optimize pose first
            self.optimize_pose_only2(th_scan_meshes, smpl, pose_iterations, pose_steps_per_iter, th_pose_3d)
    ...
    def optimize_pose_only(self, th_scan_meshes, smpl, iterations,
        ...
        #                              betas=(0.9, 0.999))
        # optimizer = torch.optim.Adam([split_smpl.trans, split_smpl.top_betas], 0.02, betas=(0.9, 0.999))
        optimizer = torch.optim.Adam([split_smpl.trans], 0.02, betas=(0.9, 0.999))
        ...
        # for it in range(3 * iter_for_global + iterations):
            loop = tqdm(range(steps_per_iter))
            ...
            # if it < 2 * iter_for_global:
            #     # Optimize global orientation
            #     print('Optimizing SMPL global translation')
            #     loop.set_description('Optimizing SMPL global translation')
            # # elif it < 3 * iter_for_global:
            # else:
            #     # Optimize global orientation
            #     print('Optimizing SMPL top betas')
            #     loop.set_description('Optimizing SMPL top betas')
            #     optimizer = torch.optim.Adam([split_smpl.top_betas], 0.02, betas=(0.9, 0.999))
            # elif it < 2 * iter_for_global: # added code
            # else:  # added code
            #     # Optimize global orientation
            #     print('Optimizing SMPL global orientation')
            #     loop.set_description('Optimizing SMPL global orientation')
            #     # optimizer = torch.optim.Adam([split_smpl.trans, split_smpl.top_betas, split_smpl.global_pose], 0.02,
            #     #                              betas=(0.9, 0.999))
            #     optimizer = torch.optim.Adam([split_smpl.global_pose], 0.02, betas=(0.9, 0.999))
            # elif it == 2 * iter_for_global:
            #     # Now optimize full SMPL pose
            #     print('Optimizing SMPL pose only')
            #     loop.set_description('Optimizing SMPL pose only')
            #     optimizer = torch.optim.Adam([split_smpl.trans, split_smpl.top_betas, split_smpl.global_pose,
    ...
    def optimize_pose_only2(self, th_scan_meshes, smpl, iterations, steps_per_iter, th_pose_3d, prior_weight=None):
        # split_smpl = SMPLHPyTorchWrapperBatchSplitParams.from_smplh(smpl).to(self.device)
        split_smpl = SMPLPyTorchWrapperBatchSplitParams.from_smpl(smpl).to(self.device)
        optimizer = torch.optim.Adam([split_smpl.body_pose], 0.02, betas=(0.9, 0.999))
        ...
        # for it in range(3 * iter_for_global + iterations):
            loop = tqdm(range(steps_per_iter))
            print('Optimizing SMPL params')
            loop.set_description('Optimizing SMPL params')
