...
class SMPLDFitter(SMPLHFitter):
    def __init__(self, model_root, device='cuda:6', save_name='smpld', debug=False, hands=False):
...
if __name__ == "__main__":
    ...
    args.scan_path = glob.glob('../data/01_20220527_JMG_origin/meshes/*.ply')
    args.pose_file = glob.glob('../data/01_20220527_JMG_origin/keypoints3d/*.json')
    args.smpl_pkl = glob.glob("../data/01_20220527_JMG_origin/SMPL_pkls/*.pkl")
    ...
    args.save_path = '../data/01_20220527_JMG_origin_result'
    args.gender = 'neutral'
    config = load_config(args.config_path)
    args.model_root = Path(config["SMPL_MODELS_PATH"])
